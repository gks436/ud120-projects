...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
	    ...
	    self.epsilon=math.exp(-1*(self.n_trial)*(0.0009))
        return None
    ...
    def get_maxQ(self, state):
        ...
        a=max(self.Q[state].iterkeys(),key=(lambda key:self.Q[state][key]))
        maxQ=self.Q[state][a]
        return maxQ
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            c=[k for k,v in self.Q[state].items() if v==self.get_maxQ(state)]
...
def run():
    ...
    sim.run(tolerance=0.04,n_test=30)
